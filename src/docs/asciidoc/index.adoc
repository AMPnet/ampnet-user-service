...
=== Verify account
To verify account user must complete KYC process and send verification data `session_id` received from KYC provider.
Verification overrides user's first and last name defined in registration with verified data.
